...
class CubeUpdateSensor(BaseSensorOperator):
    @apply_defaults
    def __init__(self, tm1_conn_id, cube_name, *args, **kwargs):
        if kwargs.get('timeout') is None:
            kwargs['timeout'] = 60
        if kwargs.get('poke_interval') is None:
            kwargs['poke_interval'] = 5
        kwargs['soft_fail'] = True
        kwargs['task_id'] = f'{tm1_conn_id.replace(' ', '')}-{cube_name.replace(' ', '')}-LastUpdateSensor'
    ...
    def poke(self, context):
        with self.hook.get_conn() as tm1:
            ...
            if last_update != last_flag:
              Variable.set(variable_name, last_update,

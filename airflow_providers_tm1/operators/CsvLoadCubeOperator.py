...
class CsvLoadCubeOperator(BaseOperator):
    ...
    def __init__(
        self, connection_id, csv_file_path: str, cube_name: str, transform_callable: Callable | None, *args, **kwargs
    ):

        super().__init__(
            *args,
            task_id=f"load-csv-to-{cube_name.replace(' ', '')}",
            task_display_name=f"Load {os.path.basename(csv_file_path)} to {connection_id}.{cube_name}",
            **kwargs,
        )
        ...
        with self.hook.get_conn() as tm1:
            ...
            if not self.healthcheck(tm1, df):
                raise AirflowException(
                    f"Healthcheck failed for cube {self.cube_name}, please check the logs for more information"
                )
